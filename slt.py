...
def df_from_tar(name: str):
    ...
    if tar_file:
        if name in tar_file.getnames():
            return pd.read_csv(BytesIO((tar_file.extractfile(name)).read())).rename(columns=str.lower)
        elif './'+name in tar_file.getnames():
            return pd.read_csv(BytesIO((tar_file.extractfile('./'+name)).read())).rename(columns=str.lower)
        else:
            print('压缩包中找不到对应文件: ',name)
            return None
    else:
        st.error('上载的文件包有误。')
        st.stop()
...
def util_gen_awr_section(section_name,
                         section_anchor,
                         table_name,
                         stat_column,
                         value_column,
                         ...
                         value_type='wait_time_delta'):
    st.subheader(section_name, anchor=section_anchor)
    df: pd.DataFrame = query_awr_generic(
        st.session_state.selected_conn,
        datetime.datetime.now() - datetime.timedelta(1),
        datetime.datetime.now(), table_name, stat_column, value_column,
        is_cdb) if not file_uploaded else df_from_tar(section_anchor)
    if df is not None:
        ...
        for con_name in df['name'].unique():
            ...
            # Top-N gets
            var_ser = df_con.groupby(
                ['instance_number',
                 'stat_name'])['waits_var'].mean().sort_values().tail(n=15)
            var_ser = var_ser.reset_index()
            top_n_stats: list = (
                var_ser['stat_name'] + '@' +
                var_ser['instance_number'].astype(str)).tolist()
            if include_stats:
                for inst_num in df_con['instance_number'].unique():
                    top_n_stats = top_n_stats + [
                        x + '@' + str(inst_num) for x in include_stats
                    ]

            not_in_stats = []
            if exclude_stats:
                for inst_num in df_con['instance_number'].unique():
                    not_in_stats = not_in_stats + [
                        x + '@' + str(inst_num) for x in exclude_stats
                    ]
            date_index = df_con['begin_interval_time'].sort_values().unique(
            ).tolist()
            # date_index
            df_con['stat_name'] = df_con['stat_name'] + '@' + df_con[
                'instance_number'].astype(str)
            df_con = df_con[['begin_interval_time', 'stat_name', value_type]]
            # df_con
            df_con = df_con.pivot_table(index='begin_interval_time',
                                        columns='stat_name',
                                        values=value_type,
                                        fill_value=0)
            ...
            option = {
                'title': {
                    'text':
                    con_name +
                    f' {section_name} (AWR快照间隔内[默认1小时],单位 {unit})， 数据来源AWR'
                },
                'tooltip': {
                    'trigger': 'axis'
                },
                ...
                'series': [
                    {
                        'name': col,
                        'data': df_con[col].values.tolist(
                        ),  #grp_data['wait_time_delta'].fillna(method='ffill').values.tolist(),
                        'type': 'line',
                        'smooth': True
                    } for col in df_con
                    if col in top_n_stats and col not in not_in_stats
...
def render_topsql_section(df: pd.DataFrame, is_cdb: bool,
                          df_pdb: pd.DataFrame):
    if df is not None:
        if is_cdb:
            df = df.merge(df_pdb[['con_id', 'name']], on='con_id')
...
st.sidebar.selectbox(label='选择连接',
                     options=st.session_state.connections_defined.keys(),
                     key='selected_conn')
...
with profile_placeholder.container():
    df_inst = query_inst(st.session_state.selected_conn
                         ) if not file_uploaded else df_from_tar('instance')
    if df_inst is not None:
        ...
        print('Instance info cannot be None.')
        st.stop()

    # v$database
    db_df: pd.DataFrame = query_database(
        st.session_state.selected_conn) if not file_uploaded else df_from_tar(
            'database')
    if not file_uploaded:
    ...
    if db_df is not None:
        ...
        is_archivelog = (db_df['LOG_MODE'.lower()][0] == 'ARCHIVELOG')
        is_cdb = version_12c and (db_df['CDB'.lower()][0] == 'YES')
        if is_cdb:
            df_pdb: pd.DataFrame = query_pdb(
                st.session_state.selected_conn
            ) if not file_uploaded else df_from_tar('pdb-info')
            if not file_uploaded:
                df2tar(df_pdb, tar_file, 'pdb-info.csv')

            df_pdb = df_pdb.append(
                pd.DataFrame([[0, 'Entire CDB']],
                             columns=['con_id',
                                      'name'])).rename(columns=str.lower)
            with pdb_choose_container.container():
                st.selectbox(label='选择PDB',
                             options=pd.Series(['All']).append(df_pdb['name']),
                             key='selected_pdb')

        db_df.rename(columns={
            'name': '数据库名',
            'log_mode': '归档模式',
            'CONTROLFILE_TYPE'.lower(): '控制文件类型',
            'OPEN_MODE'.lower(): '打开模式',
            'PROTECTION_MODE'.lower(): '保护模式',
            'PROTECTION_LEVEL.lower()': '保护级别',
            'DATABASE_ROLE'.lower(): '数据库角色',
            'SUPPLEMENTAL_LOG_DATA_MIN'.lower(): '补充日志最小',
            'SUPPLEMENTAL_LOG_DATA_PK'.lower(): '补充日志主键',
            'SUPPLEMENTAL_LOG_DATA_UI'.lower(): '补充日志UI',
            'FORCE_LOGGING'.lower(): '强制日志',
            'PLATFORM_NAME'.lower(): '平台名称',
            'FLASHBACK_ON'.lower(): '闪回',
            'SUPPLEMENTAL_LOG_DATA_FK'.lower(): '补充日志FK',
            'SUPPLEMENTAL_LOG_DATA_ALL'.lower(): '补充日志ALL'
        },
                     inplace=True)
        # db_df = db_df.melt(id_vars=('DBID', '数据库名'), var_name='配置', value_name='值')
        for col in db_df.columns:
            db_df[col] = db_df[col].astype(str)
        df2 = db_df.melt(id_vars=('dbid', '数据库名'),
                         var_name='配置',
                         value_name='值')
        # st.write(df2.info())
    ...
    sunburst_data = []
    layer_df(data=df_mem,
             parent_column='parent_memory_component',
             id_column='memory_component',
             name_label='name',
             value_label='value',
             name_property='memory_component',
             value_property='memory_size',
             store_dict=sunburst_data)
    # sunburst_data
    ...
    if is_archivelog:
        ...
        df_arl = query_arl_dest(
            st.session_state.selected_conn
        ) if not file_uploaded else df_from_tar('arl-dest-info')
        if not file_uploaded:
        ...
        df_bakup = query_rman_bakinfo(
            st.session_state.selected_conn
        ) if not file_uploaded else df_from_tar('backup-info')
        if not file_uploaded:
    ...
    df_fra = query_fra(st.session_state.selected_conn
                       ) if not file_uploaded else df_from_tar('fra-info')
    if not file_uploaded:
    ...
    if is_cdb:
        df_users = df_users.merge(df_pdb[['con_id', 'name']],
                                  on='con_id').fillna('')
        _s = filter_pdb(df_users, 'name')

        df_users = df_users.loc[_s, [
...
if show_storage_file:
    ...
    st.header('重做日志文件')
    df_redo = query_logfile(st.session_state.selected_conn
                            ) if not file_uploaded else df_from_tar('redo-log')
    if not file_uploaded:
    ...
    if df_dbfile is not None:
        if is_cdb:
            ...
            df_dbfile = df_dbfile.loc[_s, [
                'name_y', 'name_x', 'file#', 'bytes', 'status', 'open_mode',
                'enabled', 'ts#'
            ]]
    ...
    if df_ptbspc is not None:
        if is_cdb:
            df_ptbspc = df_ptbspc.merge(df_pdb[['con_id', 'name']],
                                        on='con_id')
            _s = filter_pdb(df_ptbspc, 'name')

            df_ptbspc = df_ptbspc.loc[_s, [
                'name', 'tablespace_name', 'file_size', 'file_max_size',
                'file_free_space'
            ]]
        else:
            df_ptbspc = df_ptbspc['tablespace_name', 'file_size',
                                  'file_max_size', 'file_free_space']
        df_ptbspc['file_size'] = df_ptbspc['file_size'].apply(
    ...
    if df_utbspc is not None:
        if is_cdb:
            df_utbspc = df_utbspc.merge(df_pdb[['con_id', 'name']],
                                        on='con_id')
            ...
            df_utbspc = df_utbspc['tablespace_name', 'file_size',
                                  'file_max_size', 'file_free_space']
    ...
    if df_ttbspc is not None:
        if is_cdb:
            df_ttbspc = df_ttbspc.merge(df_pdb[['con_id', 'name']],
                                        on='con_id')
            ...
            df_ttbspc = pd.pivot_table(df_ttbspc,
                                       values='value',
                                       index=['name', 'tablespace_name'],
                                       columns='param')
        else:
            df_ttbspc = df_ttbspc['tablespace_name', 'param', 'value', ]
            df_ttbspc = pd.pivot_table(df_ttbspc,
                                       values='value',
    ...
    if st.checkbox('显示冗余的索引(需要能够访问sys用户基表权限，结果仅供参考，调整应在专家指导下进行)'):
        df_rdnt_idx = query_redundent_idx(
            st.session_state.selected_conn
        ) if not file_uploaded else df_from_tar('redundent-indexes')
        if not file_uploaded:
...
if show_sql_info:
    ...
    with sqlprof_placeholder.container():
        ...
        if df_sqlarea is not None:
            if is_cdb:
                df_sqlarea = df_sqlarea.merge(df_pdb[['con_id', 'name']],
                                              on='con_id')
                _s = filter_pdb(df_sqlarea, 'name')

                df_sqlarea = df_sqlarea.loc[_s, [
                    'name', 'instance_name', 'total_sql', 'totalsqlmem',
                    'singleusesql', 'singleusesqlmem', 'totalcurs', 'cputime',
                    'elapsetime'
                ]]
            else:
                df_sqlarea = df_sqlarea[[
                    'instance_name', 'total_sql', 'totalsqlmem',
                    'singleusesql', 'singleusesqlmem', 'totalcurs', 'cputime',
                    'elapsetime'
                ]]
    ...
    with topsql_placeholder.container():
        ...
        if st.checkbox('TOP SQL: 按照整体执行次数[executions]排序'):
            df_tsql_exec = query_topsql_exec(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照整体排序次数[sorts]排序'):
            df_tsql_sort = query_topsql_sorts(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-sort')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照占用共享内存[sharable_mem]排序'):
            df_tsql_mem = query_topsql_shm(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-shmemory')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照并发执行次数[px server executions]排序'):
            df_tsql_px = query_topsql_pxexec(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-pxexec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照失效次数[invalidations]排序'):
            df_tsql_invld = query_topsql_invalid(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-invalids')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照SQL解析次数[parse]排序'):
            df_tsql_prs = query_topsql_parse(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-parse')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照磁盘读取次数[disk reads]排序'):
            df_tsql_phr = query_topsql_phr(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-phr-reads')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照内存读取次数[buffer gets]排序'):
            df_tsql_get = query_topsql_get(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-buffer-get')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照应用程序等待时间[application wait time]排序'):
            df_tsql_awt = query_topsql_awt(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-app-wait')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照并发等待时间[concurrency wait time(锁资源)]排序'):
            df_tsql_conwt = query_topsql_conwt(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-concurrent-wait')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照集群等待时间[cluster wait time]排序'):
            df_tsql_cluwt = query_topsql_cluwt(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-cluster-wait')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照用户磁盘IO等待时间[user io wait time]排序'):
            df_tsql_uiowt = query_topsql_uiowt(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-user-iowait')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照操作数据行数[rows processed]排序'):
            df_tsql_rows = query_topsql_rows(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-rows')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照消耗cpu时间[CPU time]排序'):
            df_tsql_cpu = query_topsql_cpu(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-cpu')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照执行时间[elapsed time]排序'):
            df_tsql_elps = query_topsql_elps(
                st.session_state.selected_conn,
                is_cdb) if not file_uploaded else df_from_tar('topsql-elapse')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均排序次数[sorts/exec#]排序'):
            df_tsql_sort_pe = query_topsql_sortspe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-sort-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均解析次数[parse/exec#]排序'):
            df_tsql_prs_pe = query_topsql_parsepe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-parse-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均磁盘读取次数[disk reads/exec#]排序'):
            df_tsql_phr_pe = query_topsql_phrpe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-phread-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均内存读取次数[buffer gets/exec#]排序'):
            df_tsql_get_pe = query_topsql_getpe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-gets-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均应用程序等待时间[application wait time/exec#]排序'):
            df_tsql_awt_pe = query_topsql_awtpe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-appwait-exec')
            if not file_uploaded:
                df2tar(df_tsql_awt_pe, tar_file, 'topsql-appwait-exec.csv')
            render_topsql_section(df_tsql_awt_pe, is_cdb, df_pdb)

        if st.checkbox(
                'TOP SQL: 按照平均并发等待时间[concurrency wait time/exec#(锁资源)]排序'):
            df_tsql_conwt_pe = query_topsql_conwt_pe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-concurrwait-exec')
            if not file_uploaded:
                df2tar(df_tsql_conwt_pe, tar_file,
                       'topsql-concurrwait-exec.csv')
        ...
        if st.checkbox('TOP SQL: 按照平均集群等待时间[cluster wait time/exec#]排序'):
            df_tsql_cluwt_pe = query_topsql_cluwt_pe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-clustwait-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均用户磁盘IO等待时间[user io wait time/exec#]排序'):
            df_tsql_uiowt_pe = query_topsql_uiowt_pe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-userio-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均操作数据行数[rows processed/exec#]排序'):
            df_tsql_row_pe = query_topsql_rows_pe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-rows-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均消耗cpu时间[CPU time/exec#]排序'):
            df_tsql_cpu_pe = query_topsql_cpu_pe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-cpu-exec')
            if not file_uploaded:
        ...
        if st.checkbox('TOP SQL: 按照平均执行时间[elapsed time/exec#]排序'):
            df_tsql_elps_pe = query_topsql_elps_pe(
                st.session_state.selected_conn, is_cdb
            ) if not file_uploaded else df_from_tar('topsql-elapsed-exec')
            if not file_uploaded:
...
if show_io_statistics:
    ...
    with fileio_placeholder.container():
        ...
        if df_tfio is not None:
            if is_cdb:
                df_tfio = df_tfio.merge(df_pdb[['con_id', 'name']],
                                        on='con_id')
...
if show_current_sessions:
    ...
    if df_c_sess is not None:
        sess_status = st.selectbox(label='选择会话状态',
                                   options=['ALL', 'ACTIVE', 'INACTIVE'])
        if is_cdb:
            df_c_sess = df_c_sess.merge(df_pdb[['con_id', 'name']],
                                        on='con_id')
            _s = filter_pdb(df_c_sess, 'name')
            df_c_sess = df_c_sess.loc[_s, [
                'name', 'inst_id', 'sid', 'SERIAL#'.lower(
                ), 'username', 'status', 'server', 'schemaname', 'osuser',
                'process', 'machine', 'port', 'program', 'sql_id',
                'sql_child_number', 'sql_exec_start', 'prev_sql_id',
                'prev_child_number', 'prev_exec_start', 'ROW_WAIT_OBJ#'.lower(
                ), 'ROW_WAIT_FILE#'.lower(), 'logon_time',
                'blocking_session_status', 'blocking_instance',
                'blocking_session', 'final_blocking_session_status',
                'final_blocking_instance', 'final_blocking_session', 'event',
                'p1text', 'p1', 'p2text', 'p2', 'wait_class'
            ]]
        else:
            columns = [
                'name', 'inst_id', 'sid', 'SERIAL#'.lower(), 'username',
                'status', 'event', 'p1text', 'p1', 'p2text', 'p2',
                'wait_class', 'sql_id', 'ROW_WAIT_OBJ#'.lower(),
                'ROW_WAIT_FILE#'.lower(), 'logon_time',
                'blocking_session_status', 'blocking_instance',
                'blocking_session', 'final_blocking_session_status',
                'final_blocking_instance', 'final_blocking_session'
            ] if main_release >= 11 else [
                'name',
                'sid',
                'SERIAL#'.lower(),
                'username',
        ...
        df_c_sess['blocking_instance'] = df_c_sess['blocking_instance'].fillna(
            0).astype('int32')
        df_c_sess['blocking_session'] = df_c_sess['blocking_session'].fillna(
            0).astype('int32')
        df_c_sess['SERIAL#'.lower()] = df_c_sess['SERIAL#'.lower()].astype(
            'int32')
            ...
            with col1:
                ...
                options = {
                    ...
                    "series": [{
                        ...
                        [{
                            "value": item['cnt'],
                            "name": item['username'] + '-' + item['machine']
        ...
        if ('blocking_instance' in df_c_sess.columns.values.tolist()) \
            ...
            df_c_sess['info'] = df_c_sess['name']+',实例:' + \
                df_c_sess['inst_id'].astype(str)+',sid/serial#:'+df_c_sess['sid'].astype(str)+','+df_c_sess['SERIAL#'.lower()].astype(str)+',\n用户名:'+df_c_sess['username']+'状态:'+df_c_sess['status'] +'\n等待:' +df_c_sess['event']+'SQL:'+df_c_sess['sql_id']
            df_c_sess['uid'] = df_c_sess['inst_id'].astype(
                str) + '_' + df_c_sess['sid'].astype(str)
            df_c_sess['pid'] = df_c_sess.apply(
                lambda x: str(x['blocking_instance']) + '_' + str(x[
                    'blocking_session']) if x['blocking_session'] else None,
                axis=1)
            # (df_c_sess['blocking_instance'].astype(str)+'_'+df_c_sess['blocking_session'].astype(str)
            # )
            store_data = []
            layer_df(data=df_c_sess[[
                'uid', 'pid', 'info', 'sid', 'SERIAL#'.lower()
            ]],
                     parent_column='pid',
                     id_column='uid',
                     name_label='name',
    ...
    if st.checkbox('显示当前未提交事务信息(如果发现有会话状态为INACTIVE,应引起注意)'):
        df_curr_tx: pd.DataFrame = query_uncommit_tx(
            st.session_state.selected_conn,
            is_cdb) if not file_uploaded else df_from_tar('uncommit-tx')
        if not file_uploaded:
            df2tar(df_curr_tx, tar_file, 'uncommit-tx.csv')

        if df_curr_tx is not None:
            if is_cdb:
                df_curr_tx = df_curr_tx.merge(df_pdb[['con_id', 'name']],
                                              on='con_id')
                _s = filter_pdb(df_curr_tx, 'name')
                df_curr_tx = df_curr_tx.loc[_s, [
                    'name',
                    'inst_id',
                    'sid',
                    'start_time',
                ...
                df_curr_tx = df_curr_tx[[
                    'inst_id', 'sid', 'start_time', 'username', 'status',
                    'sess_status', 'ubablk'
                ]]
            st.write(
                df_curr_tx.rename(
...
if show_awr_loads:
    ...
    df_load: pd.DataFrame = query_perf_load(
        ...
        datetime.timedelta(7), datetime.datetime.now(),
        is_cdb) if not file_uploaded else df_from_tar('database-perf-load')
    if not file_uploaded:
    ...
    if df_load is not None:
        if is_cdb:
            ...
            df_load = df_load.loc[_s,
                                  ['name', 'ten_min_timestr', 'sess_count']]
        else:
    ...
    df_ash_tmodel = query_ash_timemodel(
        st.session_state.selected_conn,
        datetime.datetime.now() -
        datetime.timedelta(7), datetime.datetime.now(),
        is_cdb) if not file_uploaded else df_from_tar('ash-time-model')
    if not file_uploaded:
        df2tar(df_ash_tmodel, tar_file, 'ash-time-model.csv')
    if df_ash_tmodel is not None:
        if is_cdb:
            df_ash_tmodel = df_ash_tmodel.merge(df_pdb[['con_id', 'name']],
                                                on='con_id')
            _s = filter_pdb(df_ash_tmodel, 'name')
            df_ash_tmodel = df_ash_tmodel.loc[
                _s, ['name', 'tm', 'state', 'sess_cnt']]
        else:
        ...
        df_ash_tmodel = df_ash_tmodel.pivot_table(
            index=['tm', 'state'], columns='name',
            values='sess_cnt').reset_index().fillna(0)
        ...
        for state_value in df_ash_tmodel['state'].unique():
            data[state_value] = df_ash_tmodel[df_ash_tmodel['state'] ==
                                              state_value]
        option = {
            'tooltip': {
                'trigger': 'axis',
            ...
            'series': [
                ...
                } for state, df_state in data.items() for c in df_state
                if c not in ('tm', 'state')
            ]
        }
        # option
        st_echarts(options=option)
    ...
    if st.checkbox('显示AWR中Latch信息'):
        ...
        df_awr_latch = query_awr_latch(
            st.session_state.selected_conn,
            datetime.datetime.now() -
            datetime.timedelta(1), datetime.datetime.now(),
        ...
        if df_awr_latch is not None:
            if is_cdb:
                ...
                df_awr_latch = df_awr_latch.loc[_s, [
                    'name', 'instance_number', 'latch_name',
                    'begin_interval_time', 'wait_time', 'wait_time_delta',
                    'waits_var'
                ]]
            else:
                df_awr_latch = df_awr_latch[[
                    'name', 'instance_number', 'latch_name',
                    'begin_interval_time', 'wait_time', 'wait_time_delta',
                    'waits_var'
                ]]
            ...
            for con_name in df_awr_latch['name'].unique():
                ...
                option = {
                    'title': {
                        'text':
                        con_name + ' Latch等待(AWR快照间隔内[默认1小时],单位 us)， 数据来源AWR'
                    },
                    'tooltip': {
                        'trigger': 'axis'
                    },
                    ...
                    'xAxis': {
                        'type': 'category',
                        'data':
                        df_con['begin_interval_time'].unique().tolist()
                    },
                    'yAxis': {
                        'type': 'value'
                    },
                    ...
                    [{
                        'name':
                        _latch_name + '@' + str(_instance_number),
                        'data':
                        grp_data['wait_time_delta'].fillna(0).values.tolist(),
                        'type':
                        'line',
                        'smooth':
                        True
                    } for (_instance_number, _latch_name), grp_data in
                     df_con.groupby(['instance_number', 'latch_name'])
                     if _latch_name in top_n_stats]
                }
                st_echarts(option, height=600)
        else:
            st.warning('数据未存储')
    ...
    if st.checkbox('显示AWR中记录的后台(background)等待事件信息'):
        util_gen_awr_section('Background Event等待',
                             'awr-bg-event-wait',
                             'cdb_hist_bg_event_summary'
                             if is_cdb else 'dba_hist_bg_event_summary',
                             'event_name',
                             'TIME_WAITED_MICRO',
                             is_cdb,
                             df_pdb,
    ...
    if st.checkbox('显示AWR中记录的SGA内存信息'):
        util_gen_awr_section(
            'SGA 组件信息',
            'awr-sga-stat',
            'cdb_hist_sgastat' if is_cdb else 'dba_hist_sgastat',
            'name',
            'bytes',
            is_cdb,
            df_pdb,
            unit='us',
            value_type='wait_time')

    # bg event end.

    # time model awr, just like AWR Latch
    if st.checkbox('显示AWR中的时间模型(Time Model)数据'):
        util_gen_awr_section('Time Model等待',
                             'awr-sys-time-model',
                             'cdb_hist_con_sys_time_model'
                             if is_cdb else 'dba_hist_sys_time_model',
                             'stat_name',
                             'value',
                             is_cdb,
                             df_pdb,
    ...
    if st.checkbox('显示AWR中的RAC Misc信息'):
        util_gen_awr_section(
            'RAC MISC等待', 'awr-rac-misc',
            'CDB_HIST_DLM_MISC' if is_cdb else 'DBA_HIST_DLM_MISC', 'name',
            'value', is_cdb, df_pdb)
    ...
    if st.checkbox('显示AWR中的Enqueue锁相关信息'):
        util_gen_awr_section(
            'Enqueue等待', 'awr-enqueue-hist',
            'CDB_HIST_ENQUEUE_STAT' if is_cdb else 'DBA_HIST_ENQUEUE_STAT',
            'eq_type', 'CUM_WAIT_TIME', is_cdb, df_pdb)
    ...
    if st.checkbox('显示AWR中的数据文件IO信息:'):
        util_gen_awr_section(
            '数据文件IO', 'awr-file-statx',
            'CDB_HIST_FILESTATXS' if is_cdb else 'DBA_HIST_FILESTATXS',
            'filename', 'time', is_cdb, df_pdb)

    if st.checkbox('显示AWR中的操作系统统计信息历史:'):
        util_gen_awr_section(
            'OS Statistics History', 'os-stat-hist',
            'CDB_HIST_OSSTAT' if is_cdb else 'DBA_HIST_OSSTAT', 'stat_name',
            'value', is_cdb, df_pdb)
    if st.checkbox('显示AWR中记录的SQL执行统计信息:'):
        util_gen_awr_section(
            'SQL Statistics(执行时间) History', 'sql-stat-hist',
            'cdb_hist_sqlstat' if is_cdb else 'dba_hist_sqlstat', 'sql_id',
            'ELAPSED_TIME_TOTAL', is_cdb, df_pdb)
    if st.checkbox('显示AWR中记录的数据库统计信息:'):
        util_gen_awr_section(
            'System statistics History',
            'sys-stat-hist',
            'cdb_HIST_CON_SYSSTAT' if is_cdb else 'dba_HIST_SYSSTAT',
            'stat_name',
            'value',
            is_cdb,
            df_pdb,
            include_stats=[
                'redo writes', 'consistent reads', 'db block changes',
                'physical reads', 'physical writes', 'user calls',
                'parse count (total)', 'parse count (hard)', 'Logons',
                'user logons cumulative', 'user commits', 'user rollbacks'
            ])
    if st.checkbox('显示AWR中记录的非空闲等待事件数据:'):
        idle_events = query_event_class(
            st.session_state.selected_conn
        ) if not file_uploaded else df_from_tar('idle-events')
        if not file_uploaded:
            df2tar(idle_events, tar_file, 'idle-events')

        idle_events = idle_events[idle_events['wait_class'] == 'Idle']
        util_gen_awr_section(
            'System wait Event History',
            'sys-event-hist',
            'cdb_hist_con_system_event' if is_cdb else 'dba_hist_system_event',
            'event_name',
            'TIME_WAITED_MICRO',
            is_cdb,
            df_pdb,
            exclude_stats=idle_events['name'].values.tolist())
...
if show_jobs:
    ...
    df_auto_tasks: pd.DataFrame = query_autotasks(
        st.session_state.selected_conn,
        is_cdb) if not file_uploaded else df_from_tar('db-auto-tasks')
    if not file_uploaded:
        df2tar(df_auto_tasks, tar_file, 'db-auto-tasks.csv')
    if df_auto_tasks is not None:
        if is_cdb:
            df_auto_tasks = df_auto_tasks.merge(df_pdb[['con_id', 'name']],
                                                on='con_id')
            _s = filter_pdb(df_auto_tasks, 'name')
            df_auto_tasks = df_auto_tasks.loc[_s, [
                'name', 'client_name', 'task_name', 'operation_name', 'status',
                'window_group_name', 'enabled', 'next_start_date'
            ]]
        else:
            df_auto_tasks = df_auto_tasks[[
...
if not file_uploaded:
    st.sidebar.download_button(
        ...
        data=gen_tar_data,
        file_name=
        f"{st.session_state.selected_conn}-{datetime.datetime.strftime(datetime.datetime.now(), '%y-%m-%d_%H-%M')}-.tar.gz",
